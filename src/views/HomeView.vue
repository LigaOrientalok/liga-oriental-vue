...
const subirMediaArchivo = ref(null)
const subirMediaPreview = ref(null)
const subiendoMedia = ref(false)
...
function onMediaFile(e) {
  ...
  if (!file) return
  subirMediaArchivo.value = file
  const reader = new FileReader()
  reader.onload = ev => { subirMediaPreview.value = ev.target.result }
  reader.readAsDataURL(file)
}

async function subirMedia() {
  if (!subirMediaTitulo.value.trim()) { toast.warning('Ingresá un título'); return }
  if (subirMediaTipo.value === 'imagen' && !subirMediaArchivo.value) { toast.warning('Seleccioná una imagen'); return }
  if (subirMediaTipo.value === 'video' && !subirMediaPreview.value?.trim()) { toast.warning('Ingresá la URL del video'); return }
  subiendoMedia.value = true
  try {
    const contenido = subirMediaTipo.value === 'imagen' ? subirMediaPreview.value : subirMediaPreview.value.trim()
    await db.createMedia(subirMediaTitulo.value.trim(), subirMediaDesc.value.trim() || null, subirMediaTipo.value, contenido)
    toast.success('✅ Media subida')
    subirMediaTitulo.value = ''
    subirMediaDesc.value = ''
  ...
  <template v-else>
    ...
    <div class="box" style="margin-top:15px;">
      ...
      <div v-if="subirMediaTitulo && auth.isApproved" style="background:var(--bg-input); border-radius:8px; padding:15px; margin-bottom:15px; border-left:4px solid #eab308;">
        ...
        <input type="text" v-model="subirMediaDesc" placeholder="Descripción" />
        <label class="label-accent">Tipo:</label>
        <select v-model="subirMediaTipo" style="padding:8px; border-radius:6px; background:var(--bg-input); color:white; border:1px solid var(--border);">
          <option value="imagen">Imagen</option>
          <option value="video">Video (URL)</option>
        </select>
        <div v-if="subirMediaTipo === 'imagen'">
          <label class="label-accent">Archivo:</label>
          <input type="file" accept="image/*" @change="onMediaFile" />
          <div v-if="subirMediaPreview" style="margin-top:8px;">
            <img :src="subirMediaPreview" style="max-width:200px; max-height:120px; border-radius:6px; border:2px solid var(--border); object-fit:cover;" />
          </div>
        </div>
        <div v-else>
          <label class="label-accent">URL del video (YouTube embed):</label>
          <input type="url" v-model="subirMediaPreview" placeholder="https://www.youtube.com/embed/..." />
        </div>
      ...
      <div v-else style="display:grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap:12px;">
        <div v-for="m in mediaItems" :key="m.id" style="background:var(--bg-input); border-radius:8px; overflow:hidden; border:1px solid var(--border); position:relative;">
          ...
          <img v-else :src="m.contenido" :alt="m.titulo" style="width:100%; aspect-ratio:16/9; object-fit:cover; cursor:pointer;" @click="window.open(m.contenido, '_blank')" />
          ...
          <button v-if="auth.isAdmin || auth.user?.id === m.uploaded_by" @click="eliminarMedia(m.id)" class="btn-mini" style="position:absolute; top:6px; right:6px; background:rgba(239,68,68,0.8); color:white; padding:2px 8px; font-size:0.7rem;">🗑️</button>
        </div>
      </div>
    </div>
  </template>
